...
public final class ControlHandler extends RequestHandler {
    @NonNull
    @Override
    ...
    public void handle(@NonNull ClientRequest request) throws Exception {
        ...
        var cmd = params.optString("cmd", "");
        if (cmd.isEmpty())
            throw new RequestException("missing cmd");
        if (!inst.runControlCommand(cmd))
            throw new RequestException("failed to send command");
    }
}
