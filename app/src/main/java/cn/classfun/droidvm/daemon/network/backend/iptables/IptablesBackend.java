...
public final class IptablesBackend extends FirewallHelper {
    ...
    public boolean applyForward(
        ...
        return applyForwardRange(
            bridge, guestIp, protocol, hostIp,
    ...
    @Override
    public boolean applyForwardRange(
        @NonNull String bridge, @NonNull String guestIp, @NonNull String protocol,
        ...
        ok &= fwd.insert(fwdSpec(bridge, guestIp, protocol, guestStart, guestEnd));
        Log.i(TAG, fmt("Apply forward %s %s:%s -> %s:%s on %s (ok=%b)",
            protocol, isSpecificHost(hostIp) ? hostIp : "*",
            portRange(hostStart, hostEnd, ':'),
            guestIp, portRange(guestStart, guestEnd, ':'), bridge, ok));
        return ok;
    }

    @Override
    public boolean removeForwardRange(
        @NonNull String bridge, @NonNull String guestIp, @NonNull String protocol,
        @Nullable String hostIp, int hostStart, int hostEnd, int guestStart, int guestEnd
    ) {
        var pre = new ChainInfo(this, "nat", null, fmt("droidvm_pre_%s", bridge));
        ...
        boolean ok = pre.delete(
            dnatSpec(protocol, hostIp, hostStart, hostEnd, guestIp, guestStart, guestEnd));
        ok &= fwd.delete(fwdSpec(bridge, guestIp, protocol, guestStart, guestEnd));
        Log.i(TAG, fmt("Remove forward %s %s:%s -> %s:%s on %s (ok=%b)",
            protocol, isSpecificHost(hostIp) ? hostIp : "*",
            portRange(hostStart, hostEnd, ':'),
            guestIp, portRange(guestStart, guestEnd, ':'), bridge, ok));
        return ok;
