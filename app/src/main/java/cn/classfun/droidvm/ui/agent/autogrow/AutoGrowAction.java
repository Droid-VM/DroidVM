...
public final class AutoGrowAction extends BaseAction {
    ...
    protected String buildActionScript() {
        ...
        var script = String.join("\n",
            ...
            "    command_log \"parted -s -f $DISK resizepart $EXTENDED_NUM 100%%\"",
            "    parted -s -f \"$DISK\" resizepart \"$EXTENDED_NUM\" 100%%",
            "    rc=$?",
            "    marker \"COMMAND:RC:PARTED_EXTENDED:$rc\"",
            "    [ \"$rc\" -eq 0 ] || fail PARTITION_GROW_FAILED",
            "fi",
            "command_log \"parted -s -f $DISK resizepart $PART_NUM 100%%\"",
            "parted -s -f \"$DISK\" resizepart \"$PART_NUM\" 100%%",
            "rc=$?",
            "marker \"COMMAND:RC:PARTED:$rc\"",
            "[ \"$rc\" -eq 0 ] || fail PARTITION_GROW_FAILED",
            "command_log \"partprobe $DISK\"",
            "partprobe \"$DISK\"",
            "rc=$?",
            "marker \"COMMAND:RC:PARTPROBE:$rc\"",
            "[ \"$rc\" -eq 0 ] || fail PARTITION_REREAD_FAILED",
            ...
            "case \"$FS_TYPE\" in",
            "    ext2|ext3|ext4)",
            "        command_log \"e2fsck -pf $LAST_PART\"",
            "        e2fsck -pf \"$LAST_PART\"",
            "        FSCK_RC=$?",
            "        marker \"COMMAND:RC:E2FSCK:$FSCK_RC\"",
            "        [ \"$FSCK_RC\" -le 1 ] || fail FILESYSTEM_CHECK_FAILED",
            "        command_log \"resize2fs $LAST_PART\"",
            "        resize2fs \"$LAST_PART\"",
            "        rc=$?",
            "        marker \"COMMAND:RC:RESIZE2FS:$rc\"",
            "        [ \"$rc\" -eq 0 ] || fail FILESYSTEM_GROW_FAILED",
            "        ;;",
            "    btrfs)",
            "        modprobe btrfs >/dev/null 2>&1 || true",
            "        mount -t btrfs -o rw \"$LAST_PART\" /mnt-autogrow || fail FILESYSTEM_MOUNT_FAILED",
            "        command_log \"btrfs filesystem resize max /mnt-autogrow\"",
            "        btrfs filesystem resize max /mnt-autogrow",
            "        rc=$?",
            "        marker \"COMMAND:RC:BTRFS_RESIZE:$rc\"",
            "        [ \"$rc\" -eq 0 ] || fail FILESYSTEM_GROW_FAILED",
            "        sync",
            ...
            "        command_log \"fsck.f2fs -f $LAST_PART\"",
            "        fsck.f2fs -f \"$LAST_PART\"",
            "        rc=$?",
            "        marker \"COMMAND:RC:F2FS_FSCK:$rc\"",
            "        [ \"$rc\" -eq 0 ] || fail FILESYSTEM_CHECK_FAILED",
            "        command_log \"resize.f2fs $LAST_PART\"",
            "        resize.f2fs \"$LAST_PART\"",
            "        rc=$?",
            "        marker \"COMMAND:RC:F2FS_RESIZE:$rc\"",
            "        [ \"$rc\" -eq 0 ] || fail FILESYSTEM_GROW_FAILED",
            "        ;;",
            "esac",
            "sync",
