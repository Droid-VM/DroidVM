...
public final class InputForwarder {
    ...
    private void sendTouchNow(@NonNull MotionEvent event, float scaleX, float scaleY) {
        ...
        try {
            EvdevEncoder.sendMultiTouch(buf, event, scaleX, scaleY);
        } catch (Exception e) {
            Log.e(TAG, "encode touch failed", e);
        } finally {
            event.recycle();
        }
        sink.write(MULTITOUCH, buf.toByteArray());
    }
    ...
    public void sendRawKeyEvent(int scanCode, boolean pressed) {
        submit("sendRawKeyEvent", () -> {
            var buf = new ByteArrayOutputStream(24);
            try {
                EvdevEncoder.sendKey(buf, (short) scanCode, pressed);
            } catch (Exception e) {
                Log.e(TAG, "encode key failed", e);
            }
            boolean ok = sink.write(KEYBOARD, buf.toByteArray());
